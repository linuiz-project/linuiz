...
const ACPI2_GUID: GUID = GUID::new(0x8868e871, 0xe4f1, 0x11d3, 0xbc22, [0x00, 0x80, 0xc7, 0x3c, 0x88, 0x81]);
...
pub trait Checksum: Sized {
    ...
    fn checksum(&self) -> bool {
        unsafe { core::ptr::slice_from_raw_parts(self as *const _ as *const u8, self.bytes_len()).as_ref().unwrap() }
            .iter()
            .sum::<u8>()
            == 0
    }
...
pub fn get_rsdp() -> &'static RSDP {
    unsafe { RSDP_PTR.get().expect("RSDP pointer has not been set").as_ref().unwrap() }
}

pub unsafe fn set_rsdp_ptr(ptr: *const RSDP) {
    RSDP_PTR.set(ptr).expect("RSDP pointer has already been set");
}
