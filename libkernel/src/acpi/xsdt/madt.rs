    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.cur_header_ptr < self.max_header_ptr {
            unsafe {
                ...
                match header.ty {
                    0x0 => Some(InterruptDevice::LocalAPIC((header_ptr as *const LocalAPIC).as_ref().unwrap())),
                    0x1 => Some(InterruptDevice::IOAPIC((header_ptr as *const IOAPIC).as_ref().unwrap())),
                    0x2 => {
                        Some(InterruptDevice::IRQSrcOverride((header_ptr as *const IRQSrcOverride).as_ref().unwrap()))
                    }
                    0x4 => {
                        Some(InterruptDevice::NonMaskableIRQ((header_ptr as *const NonMaskableIRQ).as_ref().unwrap()))
                    }
                    0x5 => Some(InterruptDevice::LocalAPICAddrOverride(
                        (header_ptr as *const LocalAPICAddrOverride).as_ref().unwrap(),
                    )),
