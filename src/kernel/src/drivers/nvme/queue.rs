...
pub struct Queue<'q, T: QueueType> {
    entries: libcommon::memory::PageAlignedBox<[MaybeUninit<T::EntryType>]>,
...
impl<'q, T: QueueType> Queue<'q, T> {
    ...
    pub fn new(
        ...
        unsafe {
            ...
            Self {
                entries: Box::new_uninit_slice_in(entry_count as usize, libcommon::memory::page_aligned_allocator()),
    ...
    pub fn get_phys_addr(&self) -> Address<Physical> {
        Address::<Physical>::new(
            libcommon::memory::global_pmgr()
                .get_mapped_to(&libcommon::memory::Page::from_ptr(self.entries.as_ptr()))
                .unwrap(),
        )
    }
}
...
//     pub fn new(reg0: &'q libcommon::memory::MMIO, queue_id: u16) -> Self {
