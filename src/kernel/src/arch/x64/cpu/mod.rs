...
fn init_registers() {
    ...
    // Set CR0 flags.
    use crate::arch::x64::registers::control::{CR0Flags, CR0};
    ...
    // Set CR4 flags.
    use crate::arch::x64::registers::control::{CR4Flags, CR4};
    ...
    if cpuid::EXT_FUNCTION_INFO.as_ref().map_or(false, cpuid::ExtendedProcessorFeatureIdentifiers::has_execute_disable)
    {
        ...
        unsafe { crate::arch::x64::registers::msr::IA32_EFER::set_nxe(true) };
    } else {
...
unsafe fn init_tables() {
    ...
    /* IDT init */
    {
        ...
        let idt = &mut *idt_ptr;
        crate::arch::x64::structures::idt::set_exception_handlers(idt);
        crate::arch::x64::structures::idt::set_stub_handlers(idt);
        idt.load_unsafe();
    }
...
pub(in crate::arch::x64) extern "sysv64" fn irq_handoff(
    ...
    crate::interrupts::get_common_interrupt_handler()(irq_number, &mut control_flow_context, &mut arch_context);
