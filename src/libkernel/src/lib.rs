...
impl<MM, CTE> BootInfo<MM, CTE> {
    ...
    pub fn new(memory_map: &[MM], config_table: &[CTE], framebuffer: Option<FramebufferInfo>) -> Self {
        Self {
    ...
    pub fn memory_map(&self) -> &[MM] {
        unsafe { core::ptr::slice_from_raw_parts(self.memory_map_ptr, self.memory_map_len).as_ref().unwrap() }
    }

    pub fn config_table(&self) -> &'static [CTE] {
        unsafe { core::ptr::slice_from_raw_parts(self.config_table_ptr, self.config_table_len).as_ref().unwrap() }
    }
    ...
    pub fn validate_magic(&self) {
        assert_eq!(self.magic, Self::MAGIC, "boot_info is unaligned, or magic is otherwise corrupted");
    }
}
