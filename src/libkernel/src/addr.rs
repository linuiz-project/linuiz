...
impl<T: AddressType> Address<T> {
    pub const fn zero() -> Self {
        Self { value: 0, phantom: PhantomData }
    }

    pub const unsafe fn new_unsafe(addr: usize) -> Self {
        Self { value: addr, phantom: PhantomData }
    }
    ...
    pub const fn align_up(self, alignment: usize) -> Self {
        Self { value: crate::align_up(self.value, alignment), phantom: PhantomData }
    }

    #[inline]
    pub const fn align_down(self, alignment: usize) -> Self {
        Self { value: crate::align_down(self.value, alignment), phantom: PhantomData }
    }
...
impl Address<Physical> {
    pub const fn new(addr: usize) -> Self {
        match addr >> 52 {
            0 => Self { value: addr, phantom: PhantomData },
    ...
    pub const fn new_truncate(addr: usize) -> Self {
        Self { value: addr & 0xFFFFFFFFFFFFF, phantom: PhantomData }
    }
...
impl core::fmt::Debug for Address<Physical> {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter.debug_tuple("Address<Physical>").field(&&format_args!("0x{:X}", self.value)).finish()
    }
}
...
impl Address<Virtual> {
    pub const fn new(addr: usize) -> Self {
        match addr >> 47 {
            0 | 0x1FFFF => Self { value: addr, phantom: PhantomData },
    ...
    #[inline]
    pub const fn new_truncate(addr: usize) -> Self {
        Self { value: (((addr << 16) as isize) >> 16) as usize, phantom: PhantomData }
    }
...
impl core::fmt::Debug for Address<Virtual> {
    fn fmt(&self, formatter: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        formatter.debug_tuple("Address<Virtual>").field(&format_args!("0x{:X}", self.value)).finish()
    }
}
