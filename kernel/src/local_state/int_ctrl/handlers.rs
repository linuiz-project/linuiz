...
pub extern "x86-interrupt" fn apit_handler(_: InterruptStackFrame) {
    let int_ctrl = crate::local_state::int_ctrl().unwrap();
    crate::local_state::clock().unwrap().tick();
    int_ctrl.counters[InterruptVector::LocalTimer as usize].fetch_add(1, Ordering::Relaxed);
    int_ctrl.end_of_interrupt();
}

pub extern "x86-interrupt" fn storage_handler(_: InterruptStackFrame) {
    crate::local_state::int_ctrl().unwrap().counters[InterruptVector::Storage as usize]
