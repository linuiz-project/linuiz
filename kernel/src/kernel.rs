...
fn init_apic_timer() {
    ...
    info!(
        "{:?}",
        libkernel::PhysAddr::new(MSR::IA32_APIC_BASE.read() * !0xFFF)
    );
    ...
    apic[InvariantAPICRegister::TimerDivisor] = APICTimerDivisor::Div16 as u32;
    ...
    apic.timer().set_masked(true);
    let window_ticks = 0xFFFFFFFF - apic[InvariantAPICRegister::TimeCurrentCount];
    debug!(
        "Determined a total of {} APIC timer ticks per {}ms.",
        window_ticks,
        crate::timer::TIMER_FREQUENCY / 1000
    );
    apic[InvariantAPICRegister::TimerInitialCount] = window_ticks as u32;
    apic[InvariantAPICRegister::TimerDivisor] = APICTimerDivisor::Div16 as u32;
